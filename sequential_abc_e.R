...
rates_model <- function(probs) {
  ...
  write.csv(biopsy,paste0("tempe/", round(probs[[2]],3), "_", round(probs[[3]],3), ".csv"))
  ...
  mito.range = list(0, 1)
  disp.range = list(1, 1)
  expected = c(0.5308, 0.1447, 0.3245)
  num.trials = 2000
  hide.param = TRUE
  ...
  for(i in 1:nrow(result)){
    filename <- paste0("tempe/", round(result[i,1], 3), "_", round(result[i,2],3), ".csv")
