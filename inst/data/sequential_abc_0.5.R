...
rates_model <- function(probs) {
  ...
  write.csv(biopsy,paste0("temp_0.5/", round(probs[[2]],3), "_", round(probs[[3]],3), ".csv"))
  ...
  for(i in 1:nrow(result)){
    filename <- paste0("temp_0.5/", round(result[i,1], 3), "_", round(result[i,2],3), ".csv")
  ...
  unlink("temp_0.5/*", recursive = TRUE)
